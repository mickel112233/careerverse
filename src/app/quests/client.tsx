...
export default function QuestsClient() {
    ...
    const renderQuestCard = (quest: Quest) => {
        ...
        const progress = isCompleted ? 100 : Math.floor(Math.random() * 80) + 10; // Mock progress for demo

        return (
            <Card key={quest.id} className={cn("flex flex-col", isCompleted && "opacity-60 bg-muted/30")}>
                <CardHeader>
                ...
                <div className="p-4 pt-0">
                    <Button className="w-full" disabled={isCompleted} onClick={() => claimReward(quest)}>
    ...
    <>
        ...
        <Tabs defaultValue="daily" className="w-full">
            ...
            <TabsContent value="daily" className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {allQuests.filter(q => q.category === 'Daily').map(renderQuestCard)}
            </TabsContent>
            <TabsContent value="weekly" className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {allQuests.filter(q => q.category === 'Weekly').map(renderQuestCard)}
            </TabsContent>
            <TabsContent value="milestone" className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {allQuests.filter(q => q.category === 'Milestone').map(renderQuestCard)}
            </TabsContent>
        </Tabs>
    </>
  );
